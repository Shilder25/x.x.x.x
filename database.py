...
class TradingDatabase:
    ...
    def get_connection(self):
        """
        Re-entrant context manager for database connections with proper transaction management.
        Uses thread-local storage to ensure each thread has its own connection.
        Tracks transaction depth to support nested calls (e.g., save_prediction → update_firm_stats).
        Only begins/commits/rollbacks at the outermost level (depth 0).
        """
        ...
        if not hasattr(self._local, 'conn') or self._local.conn is None:
            self._local.conn = sqlite3.connect(
                ...
                isolation_level=None  # Use autocommit mode, manage transactions manually
            )
        ...
        # Initialize transaction depth counter if needed
        if not hasattr(self._local, 'transaction_depth'):
            self._local.transaction_depth = 0
        
        conn = self._local.conn
        
        # Only begin transaction at outermost level
        is_outermost = self._local.transaction_depth == 0
        if is_outermost:
            conn.execute("BEGIN")
        ...
        try:
            ...
            # Decrement depth EXACTLY ONCE regardless of success or failure
            self._local.transaction_depth -= 1
            
            # Only commit/rollback at outermost level
            if is_outermost:
                if success:
                    # Commit on success
                    try:
                        ...
                        try:
                            ...
                            print(f"[DATABASE] Rollback after commit failure failed: {rollback_error}")
                        
                        # Log database lock issues specifically
                        if isinstance(commit_error, sqlite3.OperationalError) and "locked" in str(commit_error).lower():
                            print(f"[DATABASE] Database lock encountered during commit: {commit_error}")
